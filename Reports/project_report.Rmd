...
error occuring:
```{r}
# Error in optimize(fnull, interval = search.int, family = family, y = y,  : 
#   invalid 'xmin' value
...
The three different designs will be used, being the same as those tested in the case study. These will be a systematic parallel line design, a random parallel line design and a zigzag design. Examples of each design are displayed in Figure \@ref(fig:Montsurvey) below:

```{r Montsurvey, fig.show = 'hold', out.height='33%', fig.cap='Example surveys for each Design'}
...
```
As with each of the simulations run as part of the North Sea sections, 5000 simulations will be run for each design. While this is different from the 100 simulations conducted in @Buckland2015 , the increase in the number of simulations will increase the reliability of the results. 


# Modeling- Variance estimation tests

``` {r}
results.extreme <- read.csv('Results/results NS extreme.csv')

results.extreme$X <- NULL

results.extreme <-  results.extreme %>%
        `rownames<-`(c('Mean',
                 'Bias',
                 'Mean Std error of estimates',
                 'Std Dev of estimates',
                 ...
                 'Estimate Kurtosis')) %>%
        `colnames<-`(c('DSM results', 'DS results'))
kable(results.extreme,
      caption = 'North Sea Extreme density results - Line') %>%
        add_header_above(c('Statistic' = 1, 'Parallel Line Design' = 2)) %>%
        kable_styling(position = 'center',
                      latex_options = "hold_position")
```
``` {r}
results.break <- read.csv('Results/results NS break.csv')

results.break$X <- NULL

results.break <-  results.break %>%
        `rownames<-`(c('Mean',
                 'Bias',
                 'Mean Std error of estimates',
                 'Std Dev of estimates',
...
# Modeling - North Sea extreme different variance estimators:

``` {r}
results.break <- read.csv('Results/results NS Extreme O2.csv')
...
results.break <-  results.break %>%
        ...
        `colnames<-`(c('DSM results', 'DS results'))
kable(results.break,
      caption = 'North Sea Extreme density results - O2 estimator') %>%
        add_header_above(c('Statistic' = 1, 'Parallel Line Design' = 2)) %>%
        kable_styling(position = 'center',
                      latex_options = "hold_position")
```
``` {r}
results.break <- read.csv('Results/results NS Extreme R2.csv')
...
results.break <-  results.break %>%
        ...
        `colnames<-`(c('DSM results', 'DS results'))
kable(results.break,
      caption = 'North Sea Extreme density results - R2 estimator') %>%
        add_header_above(c('Statistic' = 1, 'Parallel Line Design' = 2)) %>%
        kable_styling(position = 'center',
                      latex_options = "hold_position")
```
``` {r}
results.break <- read.csv('Results/results NS Extreme S2.csv')
...
results.break <-  results.break %>%
        ...
        `colnames<-`(c('DSM results', 'DS results'))
kable(results.break,
      caption = 'North Sea Extreme density results - S2 estimator') %>%
        add_header_above(c('Statistic' = 1, 'Parallel Line Design' = 2)) %>%
        kable_styling(position = 'center',
                      latex_options = "hold_position")
```
