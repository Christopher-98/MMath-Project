...
A variety of simulations were constructed across different regions, with each simulation aiming to examine a particular area of interest. The results of these produced intriguing conclusions. DSM appears to have a positive bias, overestimating the true abundance in every simulation. For point transect designs, DSM gives little improvement over the existing distance sampling approach. However, for line transects, variance estimation using the default estimators indicated that distance sampling overestimates the variance. This lead to DSM having more precise abundance estimates, as noted by @Miller2013. Using more appropriate variance estimators was identified as a solution to this, provided the design permitted this. An alternative to this was discovered when the region was stratified, with the variance estimates in these cases being comparable between distance sampling and DSM. Overall, variance estimation identified as a key area of further research, with the potential to greatly improve the precision of both approaches.
...
Simulations will be designed and implemented to compare the accuracy and precision of abundance estimates generated through the design based approach, distance sampling (DS), and the model based approach, density surface modelling (DSM). This is to ensure that the density surface modelling approach has successfully been implemented into the simulation set-up alongside the existing distance sampling approach.
...
Where $f_k$ are the smoothed functions of the covariates and $\beta_0$ is the intercept term. By multiplying the segment area $A_j$ by the estimated probability of detection $p_j$ this gives the effective area of the segment, acting as an offset to account for different segment areas. Where distance is the only covariate in the detection function, $p_j$ is constant across all segments and therefore $\hat{p_j} = \hat{p} \forall j$. The distribution of $n_j$ can then be modelled using an over-dispersed Poisson, Negative binomial or Tweedie distribution. 
...
This section details the extensions I made to the _dsims_ package (@R-dsims) as part of this project.
...
```{r defreal, fig.align='center', fig.show = 'hold', out.height='75%', fig.cap='Example Survey realisation for the default region using the Parallel line design. Black dots highlight individuals not observed. Light blue dots represent individuals observed. Dark Blue lines represent the transects and the shaded areas either side show the area of observation out to the truncation distance. Histogram below denoted the detection distance, upon which a detection function will be fitted'}
...
results.point <- results.point %>%
        `rownames<-`(c('Mean abundance estimate',
                 'Bias (%)',
                 'Mean Std error of estimates (estimated variance)',
                 'Std Dev of estimates (true variance)',
...
```
This returned a p-value of `r test$p.value` indicating at any significance level, the null hypothesis of the difference between the estimates being zero is rejected in favour of the alternate, that the DSM estimates are greater than those of DS.
