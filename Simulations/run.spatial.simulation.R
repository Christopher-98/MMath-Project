...
run.simulation <- function(simulation, run.parallel = FALSE, max.cores = NA, counter = TRUE, ...) {
  ...
  if (run.parallel) {
    ...
    if (counter) {
        ...
        results <- pbapply::pblapply(X = as.list(1:simulation@reps), FUN = single.spatial.sim.loop, simulation = simulation, save.data = save.data, load.data = load.data, data.path = data.path, cl = myCluster, counter = FALSE)
    } else {
      
      results <- parLapply(myCluster, X = as.list(1:simulation@reps), fun = single.spatial.sim.loop, simulation = simulation, save.data = save.data, load.data = load.data, data.path = data.path, counter = FALSE)
    }
  ...
  if (!run.parallel) {
    ...
    for (i in 1:simulation@reps) {
      results <- single.spatial.sim.loop(i = i,
